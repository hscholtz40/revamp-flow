...
class AutosaveJobcardRequest extends FormRequest
{
    public function authorize(): bool
    {
        $jobcard = $this->route('jobcard');

        if ($jobcard instanceof Jobcard) {
            return $this->user()?->can('update', $jobcard) ?? false;
        }
    ...
    {
        /** @var Jobcard|null $jobcard */
        $jobcard = $this->route('jobcard');
        $companyId = $jobcard?->company_id ?? $this->user()?->getCurrentCompany()?->id;
