                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div v-if="form.customer_id && canEdit">
                            <ContactSelector
                                ...
                                :initial-contact="quoteContactInitial"
                                label="Contact"
...
const form = useForm({
    ...
    line_items: (props.quote.line_items ?? props.quote.lineItems ?? []).map((item) => ({
        ...
        supplier_id: item.supplier_id != null ? Number(item.supplier_id) : null,
...
const quoteContactInitial = computed(() => {
    const c = props.quote.contact;
    if (!c) return null;
    const customerId = Number(form.customer_id) || props.quote.customer_id;
    return {
        id: Number(c.id ?? 0),
