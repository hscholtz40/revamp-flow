...
const form = useForm({
    ...
    assigned_to_user_id: props.task.assigned_to_user_id ? String(props.task.assigned_to_user_id) : '',
    assigned_to_team_id: props.task.assigned_to_team_id ? String(props.task.assigned_to_team_id) : '',
    jobcard_id: props.task.jobcard_id ? String(props.task.jobcard_id) : '',
    scheduled_start_at: props.task.scheduled_start_at ?? '',
    scheduled_end_at: props.task.scheduled_end_at ?? '',
});

const assignmentType = ref<'user' | 'team'>(props.task.assigned_to_team_id ? 'team' : 'user');
...
<template>
    ...
    <AppLayout :breadcrumbs="[{ title: 'Tasks', href: '/tasks' }, { title: 'Edit', href: `/tasks/${props.task.id}/edit` }]">
        <div class="p-4">
            <div class="mb-6">
                <h1 class="text-2xl font-bold text-gray-900">Edit Task</h1>
                <p class="mt-1 text-sm text-gray-600">Update task details, assignment, and schedule.</p>
            </div>
