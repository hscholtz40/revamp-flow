...
const form = useForm({
    ...
    line_groups: props.deliveryNote.line_groups.length > 0
        ? props.deliveryNote.line_groups
...
const submit = () => {
    form.put(deliveryNotes.update(props.deliveryNote.id).url);
};
</script>

<template>
    <Head :title="`Edit ${props.deliveryNote.delivery_note_number}`" />

    <AppLayout :breadcrumbs="[
        { title: 'Jobcards', href: jobcards.index().url },
        { title: props.jobcard.job_number, href: jobcards.show(props.jobcard.id).url },
        { title: props.deliveryNote.delivery_note_number, href: deliveryNotes.show(props.deliveryNote.id).url },
        { title: 'Edit', href: '#' },
    ]">
        <div class="space-y-6 p-4">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Edit Delivery Note</h1>
                    <p class="text-sm text-gray-600">{{ props.deliveryNote.delivery_note_number }}</p>
                </div>
                <Link
                    :href="deliveryNotes.show(props.deliveryNote.id).url"
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="flex justify-end gap-3">
                    <Link :href="deliveryNotes.show(props.deliveryNote.id).url" class="rounded-md border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50">
                        Cancel
                    </Link>
