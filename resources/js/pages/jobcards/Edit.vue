...
const quickCreateForm = useForm(createQuickCreateCustomerDefaults());
...
const form = useForm({
    ...
    line_groups: ((props.jobcard.line_groups ?? props.jobcard.lineGroups ?? []) as any[]).map((group: any, index: number) => ({
        id: group.id,
...
const runAutosave = async () => {
    ...
    try {
        const response = await fetch(`/jobcards/${props.jobcard.id}/autosave`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                Accept: 'application/json',
